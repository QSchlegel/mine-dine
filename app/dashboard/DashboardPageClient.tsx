...
import type { DashboardInitialData } from '@/lib/server/dashboard-data'
...
export default function DashboardPageClient({ initialData }: DashboardPageClientProps) {
  ...
  const [userName, setUserName] = useState<string | null>(initialData?.userName ?? null)
  const [stats, setStats] = useState(initialData?.stats ?? {
    bookings: 0,
    upcomingBookings: 0,
  })
  const [profile, setProfile] = useState<any>(initialData?.profile ?? null)
  const [profileCompletion, setProfileCompletion] = useState<ProfileCompletionResult | null>(
    initialData?.profileCompletion ?? null
  )
