...
import type { HostDashboardInitialData } from '@/lib/server/host-dashboard-data'
...
interface HostDashboardClientProps {
  initialData: HostDashboardInitialData | null
}

export default function HostDashboardClient({ initialData }: HostDashboardClientProps) {
  const router = useRouter()
  const [application, setApplication] = useState<HostApplication | null>(initialData?.application ?? null)
  const [dinnerCount, setDinnerCount] = useState(initialData?.dinnerCount ?? 0)
  const [loading, setLoading] = useState(!initialData)
  const [showTour, setShowTour] = useState(false)
  const [hasCompletedTour, setHasCompletedTour] = useState(initialData?.hasCompletedHostTour ?? false)

  useEffect(() => {
    if (initialData) {
      if (!initialData.hasCompletedHostTour && initialData.application?.status === 'APPROVED') {
        setShowTour(true)
      }
      return
    }
