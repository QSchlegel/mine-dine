...
import type { DinnersInitialData } from '@/lib/server/dinners-data'
...
function EmptyState({ userRole }: { userRole: DinnersInitialData['userRole'] }) {
...
interface DinnersPageClientProps {
  initialData: DinnersInitialData
}

export default function DinnersPageClient({ initialData }: DinnersPageClientProps) {
  const router = useRouter()
  const [dinners, setDinners] = useState<Dinner[]>(initialData.dinners)
  const [loading, setLoading] = useState(false)
  const [activeFilter, setActiveFilter] = useState<FilterOption>('All')
  const [searchQuery, setSearchQuery] = useState('')
  const [userRole, setUserRole] = useState<DinnersInitialData['userRole']>(initialData.userRole)

  useEffect(() => {
    if (initialData.dinners.length > 0) {
      return
    }
